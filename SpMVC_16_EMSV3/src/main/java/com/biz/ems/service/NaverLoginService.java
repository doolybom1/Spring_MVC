...
public class NaverLoginService {
	...
	private final String callbackURL = "https://callor.com:12600/ems_dooly/member/naver/ok";
	...
	public String oAuthLoginGet() {
		...
		*/
		...
		/*
		 * spring 4.1에서 사용하는 UriQuery 문자열을 생성하는 class
		 */
...
//		log.debug("URL:" + apiURL);
	...
	/*
	 * 네이버에 정보 요구를 할때 사용할 토큰을 요청
	 * token을 요청할때 GET/POST method를 사용할 수 있는데 여기서는 POST를 사용해서 요청을 하겠다
	 */
	public NaverTokenVO oAuthAccessGetToken(NaverReturnAuth naverOK) {
		...
		// headers와 params에 담긴 정보를 HttpEntity 데이터로 변환
		...
		/*
		 * RestTemplate을 사용하여 네이버에 token 요청
		 */
	...
	public NaverMember getNaverMemberInfo(NaverTokenVO tokenVO) {
		...
		// header 문자열을 GET의 http header에 실어서 GET 방식으로 요청을 한다
		HttpHeaders headers = new HttpHeaders();
		headers.add("Authorization", header);
		
		/*
		 * body부분에 parameter라는 문자열을 추가하고 header부분에 위의 headers에서 설정한 형식으로 생성하고
		 *  Authorization="bear ..."
		 * 모두 문자열로 변환하여 Http protocol 데이터 구조로 변경한다. 그리고 Http protocol을 사용하여 데이터를 전송할 수 있도록 준비를 한다.
		 ...
		 */
		HttpEntity<String> request = new HttpEntity<String>("paramater", headers);
