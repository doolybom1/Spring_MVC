 ...
 * - 지금부터 이 클래스는 Entity이니 물리적 table과 연동되도록 준비하라
 * 
 * 속성중에 다음 항목이 지정되어 있으면 <prop key="hibernate.hbm2ddl.auto">create</prop>
 * Entity에 지정된 속성에 따라 table을 생성하라
 * create value : drop and create(문제 : table이 있으면 삭제하고 다시 만들어 버림)
 * 지정하지 않으면 기본값이 create
 * update로 지정을 하면 table이 없으면 생성을 하고 만약 물리적 table과 구조가 다르면 오류가 발생한다
 * 
 * ※주의※ 
 * 만약 hibernate.hbm2ddl.auto 속성을 지정하지 않으면 기본값으로 create가 지정되어 기존의 테이블을 Drop한 후 재작성해 버린다
 * 
 * none : 기본값, 아무일도 하지 않는다
 * create-only : 테이블을 새로 생성
 * drop : 삭제
 * create : drop and create
 * create-drop : 시작할때 drop and create를 수행하고 session이 끝나면 다시 drop을 한다
 * 
 * validate : 검증만하고
 * update : 스키마를 갱신, 없으면 만들고 있으면 validate 수행
 */

// emsDB의 tbl_ems 테이블과 연결하라
@Entity
...
public class EmailVO {
	 ...
	 * @GeneratedValue 속성을 GeneratedValue(strategy = GenerationType.AUTO) 로 지정하면 auto_increment를 수행하여 insert할때 자동 값 채우기가 된다
	 */
	@Id
	 ...
	 * length : 칼럼의 길이 - 지정하지 않으면 DB기본 값으로 255설정
