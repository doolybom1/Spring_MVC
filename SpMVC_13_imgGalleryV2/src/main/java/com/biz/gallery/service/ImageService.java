...
public class ImageService {
	...
	protected final ImageFileService ifSerivce;
	...
	public int insert(ImageVO imageVO) {
		...
		// 2 List가 있는지 검사하여 flag setting
		boolean yesList = fileList != null && fileList.size() > 0;
		
		// 3 리스트가 있는지 검사
		if(yesList) {
			// 4 여러개의 파일중 0번 파일을 대표파일이름으로 저장
		...
		if(yesList) {
			...
			for(String fileName : imageVO.getImg_file_upload_name()) {
			...
			ifSerivce.imageFileInsert(files, img_p_code);
		}
	 ...
	 *  VO에 담아서 전달한 값은 insert가 수행행된 후에 볼 수 있으나
	 *  seq 처럼 SQL 에서 생성된 값은 확인이 불가능하다
	 *  그러나 selectKey를 이용하면 그 전에 볼 수 있다.
	 */
	public int insert(ImageVO imageVO, String dumy) {
		...
		if(fileList != null) {
			for(String fileName : imageVO.getImg_file_upload_name()) {
				files.add(ImageFilesVO.builder().img_file_upload_name(fileName).img_file_p_code(imageVO.getImg_seq()).build());
			}
			ifSerivce.imageFileInsert(files, imageVO.getImg_seq());
		}
	...
	/*
	 *  혹시 파일이 변경이 된다면 기존 파일을 먼저 제거하고 새로운 파일로 등록한다.
	 */
	...
	public int update(ImageVO imageVO) {
		...
		if((imgVO == null)) {
			...
			log.debug("기존파일 : "+ oldImageVO.getImg_file());
...
//			imageVO.setImg_file(fileNames.get(0));
	...
	public List<ImageFilesVO> files_up(MultipartHttpServletRequest mFiles){
		...
		for (MultipartFile file : mFiles.getFiles("files")) {
			...
			// 1개의 파일을 업로드 하고
			// 저장한 파일이름은 return 받고 list에 추가한 뒤에
			
			ImageFilesVO imVO = ImageFilesVO.builder().img_file_origin_name(file.getOriginalFilename()).img_file_upload_name(fService.file_up(file)).build();
			
			fileNames.add(imVO);
		}
		
		// controller로 list를 리턴해준다
		return fileNames;
	}
